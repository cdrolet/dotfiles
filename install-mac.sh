...
if [[ $response =~ (yes|y|Y) ]];then
    ...
    sudo cp $DOTFILE_HOME/configs/hosts /etc/hosts
    ok
...
# Images
# ###########################################################
  running "Copying all images to picture folder"
  sudo cp $DOTFILE_HOME/img/* ~/pictures/ > /dev/null 2>&1

# ###########################################################
# Wallpaper
# ###########################################################
MD5_NEWWP=$(md5 $DOTFILE_HOME/img/default_wallpaper.jpg | awk '{print $4}')
...
if [[ "$MD5_NEWWP" != "$MD5_OLDWP" ]]; then
  ...
  if [[ $response =~ (yes|y|Y) ]]; then
    ...
    bot "I will backup system wallpapers in ~/pictures"
    sudo cp /System/Library/CoreServices/DefaultDesktop.jpg ~/pictures/DefaultDesktop.jpg > /dev/null 2>&1
    sudo cp /Library/Desktop\ Pictures/El\ Capitan.jpg ~/pictures/El\ Capitan.jpg > /dev/null 2>&1
    sudo cp /Library/Desktop\ Pictures/Sierra.jpg ~/pictures/Sierra.jpg > /dev/null 2>&1
    sudo cp /Library/Desktop\ Pictures/Sierra\ 2.jpg ~/pictures/Sierra\ 2.jpg > /dev/null 2>&1
    ...
    sudo cp $DOTFILE_HOME/img/default_wallpaper.jpg /System/Library/CoreServices/DefaultDesktop.jpg;
    sudo cp $DOTFILE_HOME/img/default_wallpaper.jpg /Library/Desktop\ Pictures/Sierra.jpg;
    sudo cp $DOTFILE_HOME/img/default_wallpaper.jpg /Library/Desktop\ Pictures/Sierra\ 2.jpg;
    sudo cp $DOTFILE_HOME/img/default_wallpaper.jpg /Library/Desktop\ Pictures/El\ Capitan.jpg;ok
  else
    ok "skipped"
  fi
fi
...
open "$DOTFILE_HOME/configs/Solarized Light.itermcolors";ok
running "Installing the Patched Solarized Dark theme for iTerm (opening file)"
open "$DOTFILE_HOME/configs/Solarized Dark Patch.itermcolors";ok
...
defaults write com.googlecode.iterm2 "Normal Font" -string "EnvyCodeR 12";
defaults write com.googlecode.iterm2 "Non Ascii Font" -string "EnvyCodeR 12";
ok
